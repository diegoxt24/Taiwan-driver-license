...
for idx, m in enumerate(matches):
    ...
    lines = [l.strip() for l in chunk.split('\n') if l.strip() and not l.startswith('Automobile Written') and not l.startswith('—') and not l.startswith('No.') and not l.startswith('Ans') and 'PAGEMARKER' not in l]
    ...
    # Check if question text strictly requires an image
    ...
    # Find Y range of this question block
    q_y0 = None
    q_y1 = None
    ...
    if needs_image and q_y0 is not None:
        ...
        for img_rect, xref in page_imgs:
            ...
            if dist < min_dist and dist < 120: # Must be within 120pt on the same page
                min_dist = dist
                best_xref = xref
                best_rect = img_rect
...
print(f'Parsed {len(car_qs)} Car Questions with STRICT explicit image assignment!')
