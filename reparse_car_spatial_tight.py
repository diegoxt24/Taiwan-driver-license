...
for idx, m in enumerate(matches):
    ...
    if q_y0 is not None:
        ...
        for img_rect, xref in page_imgs:
            # Distance between question Y bounds and image Y bounds
            dist = min(abs(img_rect.y0 - q_y0), abs(img_rect.y0 - q_y1), abs(img_rect.y1 - q_y0))
            if dist < min_dist and dist < 65: # Tight vertical proximity threshold
                min_dist = dist
                best_xref = xref
                best_rect = img_rect
    ...
    car_qs.append({
        ...
        'explanation': '', # To be generated by developer teacher script
        'sign_image': sign_img_path
    })

print(f'Reparsed {len(car_qs)} Car Questions with strict tight spatial image matching!')
