...
def clean_text(text):
    text = text.replace("\\'", "'").replace('\\"', '"').replace("driver\\'s", "driver's").replace('\\', '')
    text = re.sub(r'—\s*\d+\s*—', '', text)
    text = re.sub(r'Motorcycle License Written Test.* Edition', '', text, flags=re.IGNORECASE)
...
for idx, m in enumerate(matches):
    ...
    if len(opt_parts) >= 7:
        ...
        if len(sub_opts) >= 6:
            options = [
                f'({sub_opts[0]}) {clean_text(sub_opts[1])}',
                f'({sub_opts[2]}) {clean_text(sub_opts[3])}',
                f'({sub_opts[4]}) {clean_text(sub_opts[5])}'
            ]
        else:
...
for idx, m in enumerate(matches_hp):
    ...
    if not stem:
        stem = "In this hazard perception video scenario, what is the safest defensive riding action?"
...
print(f'Saved {len(full_moto_bank)} total Motorcycle questions with clean stems and extracted images!')
